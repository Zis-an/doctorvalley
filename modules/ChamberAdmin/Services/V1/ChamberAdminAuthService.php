...
namespace Modules\ChamberAdmin\Services\V1;

use Exception;
use Illuminate\Support\Facades\Auth;
use Modules\ChamberAdmin\Enums\ChamberAdminEnum;

class ChamberAdminAuthService
{
    /**
    ...
    public function authLogin(array $requestData): array
    {
        ...
        if (Auth::guard('chamber')->attempt($credentials, $requestData['remember']??0)){
            request()->session()->regenerate();
            request()->session()->regenerateToken();
            return request()->user('chamber')->toArray();
        }else{
