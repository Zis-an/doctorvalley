    ...
    <main class="myprofile" id="main-section">
        <div class="personalinfo">
            <div class="myprofile-detail">
                ...
                <div class="detail">
                    <h2 class="profile-title">Degree</h2>
                </div>
            ...
            <form action="{{ route('backoffice.degree.index') }}" method="GET">
                    ...
                    <div class="collapse" id="collapseExample">
                        <div class="searchform">
                            ...
                            <div class="mysearches">
                                <input type="search" name="degree" value="{{ request('degree') }}" placeholder="Search Degree..." class="searchfield">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- CREATE Degree FORM  START-->
            <div class="personalinfo-info">
                <div class="details align-items-start">
                    <div class="details-body w-100">
                        <!-- ADD-PERSONAL-INFORMATION -->
                        <form
                            action="{{ !empty($degree) ? route('backoffice.degree.update', $degree->id) : route('backoffice.degree.store') }}"
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="inputbox">
                                        <label for="degreename" class="inputlabel">
                                            Degree Name <span>*</span>
                                        </label>
                                        <input type="text" name="degree_name"
                                            value="{{ !empty($degree) ? $degree->degree_name : old('degree_name') }}"
                                            class="form-control" placeholder="Enter degree name" autocomplete="off">
                                        {{-- <p class="error-message d-none">This field is required</p> --}}
                                        @if ($errors->has('degree_name'))
                                            <p class="error-message">{{ $errors->first('degree_name') }}</p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="inputbox">
                                        ...
                                        <select id="selectstatus" name="status" class="form-control" autocomplete="off">
                                            ...
                                            <option value="1"
                                                {{ !empty($degree) && $degree->status == 1 ? 'selected' : '' }}>Active</option>
                                            <option value="2"
                                                {{ empty($degree) || $degree->status == 2 ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                        {{-- <p class="error-message d-none">This field is required</p> --}}
                                ...
                                <div class="col-12">
                                    <div class="checkfield">
                                        <input type="checkbox" id="has_major_course" name="has_major_course" value="1" {{  !empty($degree) && $degree->has_major_course == 1 ? 'checked' : old('has_major_course')}} class="checkinput" hidden>
                                        <label for="has_major_course" class="checklabel">Has Major Course</label>
                                        @if ($errors->has('has_major_course'))
            ...
            <!-- CREATE Degree FORM END  -->

            <!-- Degree TABLE VIEW STARTS -->
            <div class="personalinfo-info">
                <div class="details align-items-start">
                    <div class="details-body w-100">
                        <div class="table-responsive">
                            <table class="table" aria-describedby="table">
                                <thead>
                                    <tr>
                                        <th scope="col">SL</th>
                                        <th scope="col" class="text-center">Degree Name</th>
                                        <th scope="col" class="text-center">Status</th>
                                        <th scope="col" class="text-center">Has Major Course?</th>
                                ...
                                <tbody>
                                    @foreach ($degrees as $key => $degree)
                                        <tr>
                                            <th scope="row">{{ ($degrees->currentPage() - 1) * $degrees->perPage() + $loop->iteration }}</th>
                                            <td class="text-center">{{ $degree->degree_name }}</td>
                                            <td>
                                                <div class="text-center">
                                                    {{ $degree->status == 1 ? 'Active' : 'Inactive' }}
                                                </div>
                                            </td>
                                            <td>
                                                <div class="text-center">
                                                    {{ $degree->has_major_course == 1 ? 'Yes' : 'No' }}
                                                </div>
                                            </td>
                                            <td>
                                                <div class="actions">
                                                    <a href="{{ route('backoffice.degree.edit', $degree->id) }}"
                                                        class="btn-action">
                                                        <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                                            data-bs-title="Edit Degree"
                                                    ...
                                                    <button type="button" class="btn-action" data-bs-toggle="modal"
                                                        data-bs-target="#confirmModal{{ $degree->id }}">
                                                        <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                                            data-bs-title="Delete Degree"
                                                            xmlns="http://www.w3.org/2000/svg" width="16"
                                            ...
                                            <div class="modal fade pe-0" id="confirmModal{{ $degree->id }}"
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-body">
                                                            <h5 class="delete-title">Are you sure you want to delete
                                                                {{ $degree->degree_name }}?</h5>
                                                        </div>
                                                        <div class="modal-footer justify-content-end gap-3">
                                                            <form
                                                                action="{{ route('backoffice.degree.delete', $degree->id) }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('DELETE')
...
@push('after-scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (form && form.action.includes('{{ route('backoffice.degree.store') }}')) {
