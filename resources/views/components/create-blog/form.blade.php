<form action="{{ !empty($blog) ? route('blog.update', $blog->id) : route('blog.store') }}" method="POST" class="postform" enctype="multipart/form-data">
    @csrf
    @if (!empty($blog))
        @method('PUT')
    @endif
    <div class="row g-4">
                    ...
                    <div class="row g-4">
                        <div class="col-12">
                            <div class="d-flex flex-column gap-2">
                                <label for="post-title">Post Title <span>*</span> </label>
                                <input type="text" name="blog_title" value="{{ !empty($blog) ? $blog->blog_title : old('blog_title') }}" id="post-title" class="form-control"
                                       placeholder="Enter your blog title">
                                @if ($errors->has('blog_title'))
                                    <p class="error-message">{{ $errors->first('blog_title') }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="d-flex flex-column gap-2">
                                <label for="readUrl">Blog Image</label>
                                <label for="readUrl" class="postimage">
                    ...
                    <div class="d-flex flex-column gap-2">
                        ...
                        <figure class="upload-thumbnail">
                            <img id="uploadedImage"
                                 src="{{ !empty($blog) && !empty($blog->thumb_path) ? asset('storage/' . $blog->thumb_path) : asset('assets/images/uploaded/default-thumbnail.jpg') }}"
                                 alt="Uploaded Image"
        ...
        @if(!empty($blog->tags))
            @php
                // Decode the JSON tags from the blog into an array of strings
        ...
        <div class="col-12">
            <div class="inputbox">
                <label for="tags" class="inputlabel">
                    Tags <span>*</span>
                </label>
                <select class="js-example-basic-multiple wide" name="tags[]" multiple="multiple" required>
                    @if(!empty($blog) && !empty($blog->tags))
        ...
        <div class="col-12">
            <div class="d-flex flex-column gap-2">
                <label for="editor">Blog Description  <span>*</span> </label>
                <textarea name="description" id="editor">{{ !empty($blog) ? (strip_tags($blog->description)) : old('description') }}</textarea>
        ...
        <div class="col-12">
            <div class="inputbox">
                ...
                <select id="selectstatus" name="status" class="form-control" autocomplete="off">
                    ...
                    <option value="1" {{ (!empty($blog) && $blog->status == 1) || old('status') == 1 ? 'selected' : '' }}>Active</option>
                    <option value="2" {{ (!empty($blog) && $blog->status == 2) || old('status') == 2 ? 'selected' : '' }}>Inactive</option>
                </select>
