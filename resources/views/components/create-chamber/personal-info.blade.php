                    ...
                    <div class="col-md-6 col-12">
                        <div class="inputbox">
                            ...
                            <select id="divisions" name="province_id" class="wide">
                                <option class="" disabled>Select Division</option>
                                @if(!empty($provinces))
                                    @foreach ($provinces as $province)
                                        <option value="{{ $province->id }}"
                                            {{ !empty($chamber) ? ($chamber->province_id == $province->id ? 'selected' : '') : (old('province_id') == $province->id ? 'selected' : '') }}>
                                ...
                                @if(!empty($cities))
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ !empty($chamber) ? ($chamber->city_id == $city->id ? 'selected' : '') : (old('city_id') == $city->id ? 'selected' : '') }}>
                                            {{ $city->city_name }}
                                        </option>
                                    @endforeach
                                @endif
                                ...
                                @if(!empty($areas))
                                    @foreach ($areas as $area)
                                        <option value="{{ $area->id }}"
                                            {{ !empty($chamber) ? ($chamber->area_id == $area->id ? 'selected' : '') : (old('area_id') == $area->id ? 'selected' : '') }}>
                                            {{ $area->area_name }}
                                        </option>
                                    @endforeach
                                @endif
